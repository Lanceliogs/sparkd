...
static midi_input_t s_inputs[SPARK_MIDI_MAX_DEVICES];
static uint8_t s_input_count = 0;

static midi_input_t *s_find_input_by_id(int device_id)
{
    for (uint8_t i = 0; i < s_input_count; i++)
    {
        if (s_inputs[i].device_id == device_id)
            return &s_inputs[i];
    }
    return NULL;
}

static midi_input_t *s_find_input_by_pattern(const char *pattern)
{
    for (uint8_t i = 0; i < s_input_count; i++)
    {
        if (strcmp(s_inputs[i].pattern, pattern) == 0)
            return &s_inputs[i];
    }
    return NULL;
}

static int s_add_input(const char *pattern, int device_id, PortMidiStream *stream)
{
    if (s_input_count >= SPARK_MIDI_MAX_DEVICES)
    {
...
static void s_remove_input(uint8_t index)
{
    if (index < s_input_count)
    {
        s_inputs[index] = s_inputs[--s_input_count];
    }
}
...
{
    if (s_find_input_by_id(device_id))
    {
    ...
    return s_add_input(name, device_id, stream);
}

int spark_midi_open_by_name(const char *pattern)
{
    midi_input_t *existing = s_find_input_by_pattern(pattern);
    ...
    return s_add_input(pattern, id, stream);
}
...
void spark_midi_close(int device_id)
{
    for (uint8_t i = 0; i < s_input_count; i++)
    {
        if (s_inputs[i].device_id == device_id)
        {
            if (s_inputs[i].stream)
                Pm_Close(s_inputs[i].stream);
            s_remove_input(i);
            return;
        }
    }
...
int spark_midi_reconnect(void)
{
    ...
    for (uint8_t i = 0; i < s_input_count; i++)
    {
        int id = Pm_FindDevice((char *)s_inputs[i].pattern, 1);
        if (id < 0)
        {
            spark_log_debug("midi:reconnect: '%s' not found", s_inputs[i].pattern);
            continue;
        }
        ...
        if (rc != pmNoError)
        {
            spark_log_error("midi:reconnect: open '%s' failed: %s", s_inputs[i].pattern, Pm_GetErrorText(rc));
            continue;
        }

        s_inputs[i].device_id = id;
        s_inputs[i].stream = stream;
        s_inputs[i].last_activity_ms = now;
        reconnected++;
        spark_log_info("midi: reconnected '%s' (device=%d)", s_inputs[i].pattern, id);
    }
...
int spark_midi_poll(spark_midi_event_t *out, int max)
{
    ...
    for (uint8_t s = 0; s < s_input_count && count < max; s++)
    {
        if (!s_inputs[s].stream)
            continue;

        int rc = Pm_Read(s_inputs[s].stream, events, SPARK_MIDI_BUFFER_SIZE);
        if (rc < 0)
        {
            spark_log_error("midi:poll: read error on '%s'", s_inputs[s].pattern);
            Pm_Close(s_inputs[s].stream);
            s_inputs[s].stream = NULL;
            s_inputs[s].device_id = -1;
            continue;
        }
        ...
        if (rc > 0)
            s_inputs[s].last_activity_ms = spark_clock_monotonic_ms();
    }
    return count;
}
